...
CN=*.${HOST_NAME}" > "${CERT_DIR}/${HOST_NAME}.cnf"

openssl req -new -sha256 -nodes -key "${CERT_DIR}/${HOST_NAME}.key" -out "${CERT_DIR}/${HOST_NAME}.csr" -config "${CERT_DIR}/${HOST_NAME}.cnf"
...
[alt_names]
DNS.1 = *.${HOST_NAME}" > "${CERT_DIR}/${HOST_NAME}-v3.ext"

openssl x509 -req -passin pass:password -in "${CERT_DIR}/${HOST_NAME}.csr" -CA "${CERT_DIR}/rootCA.crt" -CAkey "${CERT_DIR}/rootCA.key" -CAcreateserial -out "${CERT_DIR}/${HOST_NAME}.crt" -days 36500 -sha256 -extfile "${CERT_DIR}/${HOST_NAME}-v3.ext"
